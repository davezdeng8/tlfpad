...
class FlowNet3D(nn.Module):
    def __init__(self):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=11000, radius=0.5, nsample=16, in_channel=1, mlp=[32,32,64], group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=2750, radius=1.0, nsample=16, in_channel=64, mlp=[64, 64, 128], group_all=False)
        self.sa3 = PointNetSetAbstraction(npoint=689, radius=2.0, nsample=8, in_channel=128, mlp=[128, 128, 256], group_all=False)
        self.sa4 = PointNetSetAbstraction(npoint=172, radius=4.0, nsample=8, in_channel=256, mlp=[256,256,512], group_all=False)
        ...
        self.fp = PointNetFeaturePropogation(in_channel = 256+1, mlp = [256, 256])
    ...
    def forward(self, frames):
        B, T, C, N = frames.shape
        pc1 = frames[:,-1].contiguous()
        pc2 = frames[:,-2].contiguous()
        feature1 = torch.zeros(B, 1, N).to('cuda:0')
        feature2 = torch.zeros(B, 1, N).to('cuda:0')
