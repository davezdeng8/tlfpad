...
import numpy as np
from lib import pointnet2_utils_new as pointutils
...
class PointNetSetAbstraction(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all):
        ...
        last_channel = in_channel*2   # TODO：
    ...
    def forward(self, xyz, points):
        ...
        if self.group_all == False:
            fps_idx = pointutils.furthest_point_sample(xyz_t.detach(), self.npoint)  # [B, N]
            new_xyz = pointutils.gather_operation(xyz.detach(), fps_idx)  # [B, C, N]
        else:
            new_xyz = xyz
        new_points = self.queryandgroup(xyz_t.detach(), new_xyz.transpose(2, 1).contiguous(), points) # [B, 3+C, N, S]
        ...
        for i, conv in enumerate(self.mlp_convs):
            ...
            new_points = bn(F.leaky_relu(conv(new_points), negative_slope=.2))
...
class FlowEmbedding(nn.Module):
    ...
    def forward(self, pos1, pos2, feature1, feature2):
        ...
        if self.corr_func=='concat':
            ft_rpt = feature1.view(B, -1, N, 1).repeat(1, 1, 1, self.nsample)
            feat_diff = torch.cat([feat2_grouped-ft_rpt, ft_rpt], dim = 1)
        
        feat1_new = feat_diff#torch.cat([pos_diff, feat_diff], dim = 1)  # [B, 2*C+3,N,S]
        for i, conv in enumerate(self.mlp_convs):
            bn = self.mlp_bns[i]
            # feat1_new = F.relu(bn(conv(feat1_new)))
            feat1_new = bn(F.leaky_relu(conv(feat1_new), negative_slope=.2))
...
class PointNetSetUpConv(nn.Module):
    def __init__(self, nsample, radius, f1_channel, f2_channel, mlp, mlp2, knn = True):
        ...
        last_channel = f2_channel+f1_channel
        for out_channel in mlp:
            # self.mlp1_convs.append(nn.Sequential(nn.Conv2d(last_channel, out_channel, 1, bias=False),
            #                                      nn.BatchNorm2d(out_channel),
            #                                      nn.ReLU(inplace=False)))
            self.mlp1_convs.append(nn.Sequential(nn.Conv2d(last_channel, out_channel, 1),
        ...
        for out_channel in mlp2:
            ...
            #                                      nn.ReLU(inplace=False)))            
            self.mlp2_convs.append(nn.Sequential(nn.Conv1d(last_channel, out_channel, 1),
                                                 nn.LeakyReLU(negative_slope=.2),
                                                 nn.BatchNorm1d(out_channel)))
            last_channel = out_channel
...
class PointNetFeaturePropogation(nn.Module):
    ...
    def forward(self, pos1, pos2, feature1, feature2):
        ...
        dists,idx = pointutils.three_nn(pos1_t.detach(),pos2_t.detach())
        ...
        interpolated_feat = torch.sum(pointutils.grouping_operation(feature2.detach(), idx) * weight.view(B, 1, N, -1), dim = -1) # [B,C,N,3]
        ...
        for i, conv in enumerate(self.mlp_convs):
            ...
            feat_new = bn(F.leaky_relu(conv(feat_new), negative_slope=.2))
        return feat_new